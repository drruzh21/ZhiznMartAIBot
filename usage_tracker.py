...
class UsageTracker:
    ...
    def __init__(self, user_id, user_name, logs_dir="usage_logs"):
        ...
        if 'bot_messages' not in self.usage['usage_history']:
            self.usage['usage_history']['bot_messages'] = {}

    def add_bot_message(self):
        """
        Increments the count of bot text messages sent by the bot for today.
        """
        today = str(date.today())
        if today in self.usage["usage_history"]["bot_messages"]:
            self.usage["usage_history"]["bot_messages"][today] += 1
        else:
            self.usage["usage_history"]["bot_messages"][today] = 1

        # Save the updated usage to the user file
        with open(self.user_file, "w") as outfile:
            json.dump(self.usage, outfile)

    def get_bot_message_count(self):
        """
        Returns the total number of bot text messages sent to the user.
        """
        total_messages = sum(self.usage['usage_history']['bot_messages'].values())
        return total_messages

    def get_bot_message_count_today(self):
        """
        Returns the number of bot text messages sent to the user today.
        """
        today = str(date.today())
        return self.usage['usage_history']['bot_messages'].get(today, 0)
